...
def main(args):
    ...
    file_list.sort(key= lambda x:int(x[7:-4]))
    # print("resizing pic....")
    ...
    with open('train.txt','w') as fp: 
        for i in range(len(file_list)):
            fp.write('../image_result/'+file_list[i]+"\n")


            
    print("using yolov4....")
    commands = 'cd darknet-master;./darknet detector test cfg/coco.data cfg/yolov4.cfg yolov4.weights -ext_output -dont_show -out ../json_result/result.json < ../train.txt'
    os.system(commands)
    ...
    for i in range(0,len(yolo_data)):
        ...
        im = Image.open( os.path.join( "data/val2017", f"{image_id}.jpg" ) )
        #im = Image.open()
