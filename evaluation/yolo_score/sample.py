...
import sys
sys.path.append("/home/wangph/layout2img/evaluation")
from evaluation.diversity_score import DSscore
...
def main(args):
    ...
    G_name = G_arch+str(img_size)+"_"+G_type if G_type else G_arch+str(img_size)
    netG = globals()[G_name](
        num_classes=num_classes, output_dim=3).cuda() 

    print(f'Use {G_name} in {args.model_path}')
    ...
    for idx, data in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        for r in range(repeat):
            z_obj = truncted_random(num_o=bbox.size(1), thres=thres, dim=z_dim).cuda()
            z_im = truncted_random(num_o=1, thres=thres,dim=z_dim).view(1, -1).cuda()
            with torch.no_grad():
