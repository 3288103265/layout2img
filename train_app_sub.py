...
def main(args):
    ...
    for epoch in range(args.total_epoch):
        ...
        for idx, data in enumerate(tqdm(dataloader, desc=f"epoch:{epoch}")):
            real_images, label, bbox = data
            real_images, label, bbox = real_images.to(
                device), label.long().to(device).unsqueeze(-1), bbox.float().to(device)

            # update D network
            netD.zero_grad()
            real_images, label = real_images.to(
                device), label.long().to(device)
            d_out_real, d_out_robj, d_out_robj_app, feat_sub = netD(
                real_images, bbox, label) # dp reduce via concat
            ...
            fake_images = netG(z, bbox, y=label.squeeze(dim=-1), feat_sub=feat_sub.detach())
            d_out_fake, d_out_fobj, d_out_fobj_app, _ = netD(
            ...
            d_loss = lamb_obj * (d_loss_robj + d_loss_fobj) + lamb_img * (
                d_loss_real + d_loss_fake) + lamb_app * (d_loss_robj_app + d_loss_fobj_app)
            d_loss.backward()
            d_optimizer.step()
            ...
            if (idx % 1) == 0:
                netG.zero_grad()
                g_out_fake, g_out_obj, g_out_obj_app, _ = netD(
                ...
                g_loss = g_loss_obj * lamb_obj + g_loss_fake * lamb_img + \
                    pixel_loss + feat_loss + lamb_app * g_loss_obj_app
                g_loss.backward()
                g_optimizer.step()
        ...
        if (epoch + 1) % 5 == 0:
            ...
            torch.save(netD.state_dict(), os.path.join(
                args.out_path, 'model/', 'D_%d.pth' % (epoch + 1)))
