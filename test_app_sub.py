...
from model.rcnn_discriminator_app import CombineDiscriminator128_app_sub
...
def main(args):
    ...
    if args.image_size == 128:
        netG = ResnetGenerator128_app_sub(
    ...
    netD = CombineDiscriminator128_app_sub(num_classes=num_classes).cuda()

    ckpt_path_list_G = glob.glob(os.path.join(args.model_root, 'model/G_*.pth'))
    exist_model_G = glob.glob(os.path.join(args.model_root, 'samples_*/'))
    exist_model_G = natsort.natsorted(exist_model_G)
    exist_model_G.reverse()
    print(exist_model_G)
    
    ckpt_path_list_D = glob.glob(os.path.join(args.model_root, 'model/D_*.pth'))
    
    sample_start = len(exist_model_G)
    if args.sample_start > 0:
        sample_start = args.sample_start

    test_res = []
    ckpt_path_list_G = natsort.natsorted(ckpt_path_list_G)
    ckpt_path_list_G.reverse()
    print(ckpt_path_list_G)
    
    ckpt_path_listD = natsort.natsorted(ckpt_path_list_D)
    ckpt_path_listD.reverse()

    
    ckpt_path_list_G = ckpt_path_list_G[sample_start:]
    ckpt_path_list_D = ckpt_path_list_D[sample_start:]
    
    sample_path_list = [os.path.join(args.model_root,'samples_'+ os.path.basename(
        p).split('.')[0].replace('_', '')) for p in ckpt_path_list_G]
    sample_path_list.reverse()
    
    for idx, (ckpt_path_G, ckpt_path_D, sample_path) in enumerate(zip(ckpt_path_list_G, ckpt_path_list_D, sample_path_list)):
        ckpts = [ckpt_path_G, ckpt_path_D]
        nets = [netG, netD]
        print(f'>>>[{idx+1}/{len(ckpt_path_list_G)}] --{ckpt_path_G}')
        res = test_ckpt(ckpts, nets,
                  sample_path=sample_path, dataloader=dataloader, num_o=num_o)
        print(f">>>result:{res}")
        test_res.append(res)

    res_dict = dict(zip(ckpt_path_list_G, test_res))
...
def test_ckpt(model_path_list, net_list, sample_path, dataloader, num_o, save_gt=False):
    
    for model_path, net in zip(model_path_list, net_list):
    ...
    netG, netD = net_list[0], net_list[1]
    ...
    for idx, data in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        for s_i in range(sample_num):  # sample_num=5
            ...
            fake_images = netG.forward(
                z_obj, bbox, z_im, label.squeeze(dim=-1), feat_sub=feat_sub.detach())
