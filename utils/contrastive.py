...
class Scaled_NT_Xent_loss(torch.nn.Module):
    def __init__(self, use_cosine_similarity=True):
        super(Scaled_NT_Xent_loss, self).__init__()
        self.softmax = torch.nn.Softmax(dim=-1)
        # self.mask_samples_from_same_repr = self._get_correlated_mask().type(torch.bool)
        self.similarity_function = self._get_similarity_function(
            use_cosine_similarity)
...
class Rectified_NT_Xent_loss(torch.nn.Module):
    def __init__(self, use_cosine_similarity=True):
        super(Rectified_NT_Xent_loss, self).__init__()
        self.softmax = torch.nn.Softmax(dim=-1)
        # self.mask_samples_from_same_repr = self._get_correlated_mask().type(torch.bool)
        self.similarity_function = self._get_similarity_function(
            use_cosine_similarity)
